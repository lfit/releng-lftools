...
def test_to_close_to_last_run():
    ...
    assert rdh.to_close_to_last_run(last_run, rdh.throttling_delay_seconds) == True
    last_run = orig_time - datetime.timedelta(seconds=rdh.throttling_delay_seconds-1,)
    assert rdh.to_close_to_last_run(last_run, rdh.throttling_delay_seconds) == True
...
class TestFetchNexus3Catalog:
    ...
    def test_use_next_batch_from_nexus3_3(self, datafiles):
        ...
        # first end    : multicloud/openstack-pike
        # second start : multicloud/openstack-windriver
        # second end   : sdnc-ueb-listener-image
        # third start  : multicloud/openstack-windriver
        # third end    : aaf/abrmd, rolled around, skipping all non ONAP
        data_file = os.path.join(str(datafiles), "releasedockerhub_last_run_last_nexus_repo.txt")
        ...
        assert rdh.NexusCatalog[rdh.nexus3_batch_size-1][1] == 'multicloud/openstack-pike'
        ...
        assert len(rdh.NexusCatalog) == rdh.nexus3_batch_size
        assert rdh.NexusCatalog[0][1] == 'multicloud/openstack-windriver'
        assert rdh.NexusCatalog[rdh.nexus3_batch_size-1][1] == 'sdnc-ueb-listener-image'
        rdh.store_last_nexus_repo_from_this_time(store_file)
        rdh.NexusCatalog = []
        rdh.initialize("onap")
        responses.add(responses.GET, self.url, body=self.answer, status=200)
        ...
        rdh.use_next_batch_from_nexus3(store_file)
        assert len(rdh.NexusCatalog) == rdh.nexus3_batch_size
        assert rdh.NexusCatalog[0][1] == 'search-data-service'
        assert rdh.NexusCatalog[rdh.nexus3_batch_size-1][1] == 'aaf/abrmd'
