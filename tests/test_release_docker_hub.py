...
class TestProjectClass:
    ...
    def test_ProjectClass_socket_timeout (self, responses, mocker):
        ...
        #Verify that 40 timeout's on any stage failes.
        self.nbr_exc.pull = 40
        ...
        self.nbr_exc.push = 40
        with pytest.raises(requests.HTTPError) as excinfo:
            test_proj.docker_pull_tag_push()

        self.counter.pull = self.counter.tag = self.counter.push = self.counter.cleanup = 0
        self.nbr_exc.pull = self.nbr_exc.tag = self.nbr_exc.push = 0
        self.nbr_exc.cleanup = 40
        with pytest.raises(requests.HTTPError) as excinfo:
            test_proj.docker_pull_tag_push()

        #Verify 39 timeouts and the 40 is ok per stage
        self.counter.pull = self.counter.tag = self.counter.push = self.counter.cleanup = 0
        self.nbr_exc.pull = self.nbr_exc.tag = self.nbr_exc.push = self.nbr_exc.cleanup = 39
        test_proj.docker_pull_tag_push()

        assert self.counter.pull == 40
        assert self.counter.tag == 40
        assert self.counter.push == 40
        assert self.counter.cleanup == 40
