...
class TestClusterList:
    ...
    def test_list_clusters(self, mock_cloud, sample_clusters, capsys):
        """Test listing clusters."""
        mock_cloud.list_coe_clusters.return_value = sample_clusters
        cluster.list("test-cloud", days=0)
        captured = capsys.readouterr()
        assert "old-cluster-001" in captured.out
        assert "new-cluster-001" in captured.out

    def test_list_clusters_with_filter(self, mock_cloud, sample_clusters, capsys):
        """Test listing clusters with days filter."""
        mock_cloud.list_coe_clusters.return_value = sample_clusters
        cluster.list("test-cloud", days=7)
        captured = capsys.readouterr()
        assert "old-cluster-001" in captured.out
        assert "new-cluster-001" not in captured.out


class TestClusterCleanup:
    """Test cluster cleanup command."""

    def test_cleanup_clusters(self, mock_cloud, sample_clusters, capsys):
        ...
        captured = capsys.readouterr()
        assert "Removing 1 clusters" in captured.out
