...
def test_remove_http_from_url():
    ...
    for url in test_url:
        assert on2d._remove_http_from_url(url[0]) == url[1]
...
def test_format_image_id():
    ...
    for id in test_id:
        assert on2d._format_image_id(id[0]) == id[1]
...
def test_nexus_tag_class(responses):
    ...
    test_tags = on2d.NexusTagClass (org, repo)
...
def test_docker_tag_class(responses):
    ...
    answer_invalid_tags = ["latest", "v1.0.0"]
    responses.add(responses.GET, url, body=answer, status=200)
    test_tags = on2d.DockerTagClass (org, repo)
    for tag in answer_valid_tags:
        assert tag in test_tags.valid
    for tag in answer_invalid_tags:
        assert tag in test_tags.invalid
...
class TestProjectClass:
    ...
    def mocked_docker_pull(self, nexus_image_str, count, tag, retry_text=''):
    ...
    def mocked_docker_tag(self, count, image, tag, retry_text=''):
    ...
    def mocked_docker_push(self, count, image, tag, retry_text):
        """Mocking Tag the image with proper docker name and version."""
        if not image.id == self._test_image_long_id:
            raise ValueError('Wrong image id in remove')
        if not tag in ["1.4.0","1.4.1"]:
    ...
    def mocked_docker_cleanup(self, count, image, tag, retry_text=''):
        """Mocking Tag the image with proper docker name and version."""
        if not image.id == self._test_image_long_id:
            raise ValueError('Wrong image id in remove')
    ...
    def test_ProjectClass_2_missing(self, responses, mocker):
        """Test ProjectClass"""
        mocker.patch('lftools.onap_nexus_2_docker.ProjectClass._docker_pull', side_effect=self.mocked_docker_pull)
        mocker.patch('lftools.onap_nexus_2_docker.ProjectClass._docker_tag', side_effect=self.mocked_docker_tag)
        mocker.patch('lftools.onap_nexus_2_docker.ProjectClass._docker_push', side_effect=self.mocked_docker_push)
        mocker.patch('lftools.onap_nexus_2_docker.ProjectClass._docker_cleanup', side_effect=self.mocked_docker_cleanup)
        ...
        test_proj = on2d.ProjectClass (project)
    ...
    def test_ProjectClass_socket_timeout (self, responses, mocker):
        ...
        #Verify that 10 timeout's on any stage failes.
        self.nbr_exc.pull = 10
        ...
        self.nbr_exc.push = 10
        with pytest.raises(requests.HTTPError) as excinfo:
            test_proj.docker_pull_tag_push()

        self.counter.pull = self.counter.tag = self.counter.push = self.counter.cleanup = 0
        self.nbr_exc.pull = self.nbr_exc.tag = self.nbr_exc.push = 0
        self.nbr_exc.cleanup = 10
        with pytest.raises(requests.HTTPError) as excinfo:
            test_proj.docker_pull_tag_push()

        #Verify 9 timeouts and the 10 is ok per stage
        self.counter.pull = self.counter.tag = self.counter.push = self.counter.cleanup = 0
        self.nbr_exc.pull = self.nbr_exc.tag = self.nbr_exc.push = self.nbr_exc.cleanup = 9
        test_proj.docker_pull_tag_push()

        assert self.counter.pull == 10
        assert self.counter.tag == 10
        assert self.counter.push == 10
        assert self.counter.cleanup == 10
...
class TestFetchNexus3Catalog:
    ...
    def test_get_all_onap(self):
        ...
        on2d.get_nexus3_catalog ('onap')
        assert len(on2d.NexusCatalog) == 203

    def test_get_all_onap_and_filter_1(self):
        on2d.NexusCatalog = []
        responses.add(responses.GET, self.url, body=self.answer, status=200)
        on2d.get_nexus3_catalog ('onap', 'spike')
        assert len(on2d.NexusCatalog) == 1
        assert on2d.NexusCatalog[0][0] == 'onap'
        assert on2d.NexusCatalog[0][1] == 'spike'

    def test_get_all_onap_and_filter_18(self):
        on2d.NexusCatalog = []
        responses.add(responses.GET, self.url, body=self.answer, status=200)
        on2d.get_nexus3_catalog ('onap', 'aaf')
        assert len(on2d.NexusCatalog) == 18
...
class TestFetchAllTagsAndUpdate:
    ...
    def initiate_test_fetch(self, responses):
        url = 'https://nexus3.onap.org:10002/v2/_catalog'
    ...
    def test_fetch_all_tags(self, responses, mocker):
        self.initiate_test_fetch(responses)
        on2d.get_nexus3_catalog ('onap')
        on2d.fetch_all_tags()
        assert len(on2d.NexusCatalog) == 3
        assert len(on2d.projects) == 3
        assert len(on2d.projects[0].tags_2_copy.valid) == 1
        assert len(on2d.projects[1].tags_2_copy.valid) == 0
        assert len(on2d.projects[2].tags_2_copy.valid) == 2

        assert on2d.projects[0].tags_2_copy.valid[0] == '1.4.0'
        assert on2d.projects[2].tags_2_copy.valid[0] == '1.3.1'
        assert on2d.projects[2].tags_2_copy.valid[1] == '1.3.2'
    ...
    def test_copy(self, responses, mocker):
        ...
        on2d.get_nexus3_catalog ('onap')
        on2d.fetch_all_tags()
        on2d.copy_from_nexus_to_docker()
    ...
    def test_start_copy(self, responses, mocker):
        ...
        mocker.patch('lftools.onap_nexus_2_docker.ProjectClass._docker_cleanup', side_effect=self.mocked_docker_cleanup)
        self.initiate_test_fetch(responses)
        on2d.start_point ('onap', '', False, False, True)
        assert len(on2d.NexusCatalog) == 3
        assert len(on2d.projects) == 3
        assert len(on2d.projects[0].tags_2_copy.valid) == 1
        assert len(on2d.projects[1].tags_2_copy.valid) == 0
