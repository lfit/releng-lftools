...
from lftools.git.gerrit import Gerrit
from lftools.git.gerrit import Repo  # Import so we can patch methods
from lftools.git.gerrit import gerrit_api
...
def mock_init(mocker, datafiles):
    ...
    Repo.clone_from.assert_called_once_with(
        "https://myname:mypass@gerrit.example.com/r/a/test",
        ciman_dir
    )
    mocker.stopall()
    return git
...
def test_add_info_job(mock_init, datafiles, mocker):
    ...
    filepath = os.path.join(mock_init.repo.working_tree_dir,
...
project:
    ...
    mock_init.add_info_job(fqdn, gerrit_project, issue_id)
