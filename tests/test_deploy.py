...
import lftools.deploy as deploy_sys2
import mock
import requests
...
def test_log_and_exit():
    ...
    with pytest.raises(SystemExit) as excinfo:
        deploy_sys2.local_log_error_and_exit("testmsg")
...
def test_get_node_from_xml():
    ...
    document = """\
        ...
        """
    assert deploy_sys2.local_get_node_from_xml(document, 'stagedRepositoryId') == '432'
    with pytest.raises(SystemExit) as excinfo:
        deploy_sys2.local_get_node_from_xml(document, 'NotFoundTag')
    assert excinfo.type == SystemExit
...
@mock.patch('lftools.deploy.local_log_error_and_exit', side_effect=mocked_log_error)
def test_local_request_post(self, mock_post):
    ...
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.local_request_post('connection.error.test', xml_doc, "{'Content-Type': 'application/xml'}")
    assert 'connection_error' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.local_request_post('invalid.url.test:8081nexus', xml_doc, "{'Content-Type': 'application/xml'}")
    assert 'invalid_url' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.local_request_post('http:/missing.schema.test:8081nexus', xml_doc, "{'Content-Type': 'application/xml'}")
...
def test_nexus_stage_repo_create(self, mock_post):
    ...
    res = deploy_sys2.nexus_stage_repo_create('valid.create.post', '93fb68073c18')
    assert res == 'test1-1030'
    with pytest.raises(ValueError) as excinfo:
        res = deploy_sys2.nexus_stage_repo_create('profile.id.not.exist', 'INVALID')
    assert 'profile.id.not.exist' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        res = deploy_sys2.nexus_stage_repo_create('other.create.error', 'INVALID')
    assert 'other.create.error' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        res = deploy_sys2.nexus_stage_repo_create('post.not.supported', 'INVALID')
    assert 'post.not.supported' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        res = deploy_sys2.nexus_stage_repo_create('site.not.found', 'INVALID')
    assert 'site.not.found' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        res = deploy_sys2.nexus_stage_repo_create('other.error.occured', 'INVALID')
...
def test_nexus_stage_repo_close(self, mock_post):
    ...
    deploy_sys2.nexus_stage_repo_close('valid.create.post', '93fb68073c18', 'test1-1027')
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.nexus_stage_repo_close('site.not.found', 'INVALID', 'test1-1027')
    assert 'site.not.found' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.nexus_stage_repo_close('missing.staging.repository', 'INVALID',  'test1-1027')
    assert 'missing.staging.repository' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.nexus_stage_repo_close('staging.already.closed', 'INVALID',  'test1-1027')
    assert 'staging.already.closed' in str(excinfo.value)
    with pytest.raises(ValueError) as excinfo:
        deploy_sys2.nexus_stage_repo_close('other.error.occured', 'INVALID', 'test1-1027')
    assert 'other.error.occured' in str(excinfo.value)
