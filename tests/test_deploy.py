...
def test_nexus_stage_repo_create(mocker):
...
def test_nexus_stage_repo_close(mocker):
...
def test_nexus_stage_repo_create2(responses, mocker):
    """Test nexus_stage_repo_create."""
    mocker.patch('lftools.deploy._log_error_and_exit', side_effect=mocked_log_error)
    class MockResponse:
        ...
        def json(self):
            return self.text
    created = "<promoteResponse><data><stagedRepositoryId>test1-1030</stagedRepositoryId><description>Create staging repository.</description></data></promoteResponse>"
    url = 'service/local/staging/profiles'
    print ('json1={}'.format(MockResponse(created).json()))
    print ('json2={}'.format(MockResponse(created).json()).strip('"\''))

    responses.add(responses.POST, 'http://valid.create.post/{}/{}/start'.format(url, '93fb68073c18' ),
                  json=MockResponse(created).json(), status=201)

    res = deploy_sys.nexus_stage_repo_create('valid.create.post', '93fb68073c18')
    assert res == 'test1-1030a'
