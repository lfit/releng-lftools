    ...
    def __init__(self, org_name, repo_name, repo_from_file):
        ...
        if r.status_code == 429:
            ...
            raise requests.HTTPError(
                "Throttling in effect. {} Docker pulls so far. You need to wait {} seconds and possible reduce scope. \n{}".format(
...
def print_stats():
    ...
    print_tags_header("Tag statistics (V=Valid, I=InValid)", NEXRetrieveUS3_PROJ_NAME_HEADER)
    fmt_str = "{:<" + str(project_max_len_chars) + "} : "
    for proj in projects:
        log.info(
