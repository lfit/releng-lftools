...
class Nexus:
    ...
    def create_priv(self, name, target_id, priv):
        ...
        if r.status_code != requests.codes.created:
            raise Exception("Privilege not created for '%s', code '%s'".format(name, r.status_code))
    ...
    def create_user(self, name, domain, role_id, password, extra_roles=[]):
        ...
        if user.status_code != requests.codes.created:
            raise Exception("User not created for '%s', code '%s'".format(name, user.status_code))
