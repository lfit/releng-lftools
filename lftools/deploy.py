...
def nexus_stage_repo_close(nexus_url, staging_profile_id, staging_repo_id):
    ...
    def upload_maven_file_to_nexus(nexus_url, nexus_repo_id, group_id, artifact_id, version, packaging, file,
                                   classifier=None):
        ...
        response = _request_post(url=input_url, data=params, headers=input_header)
        ...
        if response.status_code == "200":
            ...
            _log_error_and_exit(
                "failed with status code:{}, status message: []".format(response.status_code, response.text))  # Fail
