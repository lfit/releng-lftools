...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    dummy_file = open("test.txt", "a").close()
    s3_silo = s3_path.split("/")[0]
    s3_jenkins_host = s3_path.split("/")[1]
    s3 = boto3.resource("s3")
    log.info("Uploading Dummy Files")
    s3.Bucket(s3_bucket).upload_file(dummy_file, "{}{}".format(s3_silo, dummy_file))
    s3.Bucket(s3_bucket).upload_file(dummy_file, "{}{}".format(s3_jenkins_host, dummy_file))
    log.info("Deleting Dummy Files")
    # s3.delete_objects(Bucket=s3_bucket, Delete{"Objects": [{"Key":"test.txt"}]})
