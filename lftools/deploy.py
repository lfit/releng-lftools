...
def deploy_archives_s3(s3_bucket, workspace, pattern=None):
    ...
    archives_zip = shutil.make_archive(
        '{}/archives'.format(workspace), 'zip')
    log.debug('archives zip: {}'.format(archives_zip))
    deploy_s3_zip(s3_bucket, archives_zip)
...
def deploy_s3_zip(s3_bucket, zip_file, object_name="archives.zip"):
    ...
    try:
        ...
        #subprocess.run('aws s3 cp '+zip_file+ " s3://"+s3_bucket+ " --profile zow", shell=True)
