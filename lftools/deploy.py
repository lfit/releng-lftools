...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    def _tmpfile_upload(_dir, file, _ExtraArgs={}):
        try:
            if len(_ExtraArgs > 0):
    ...
    def _s3_put_file(file):
        if file in "_tmpfile":
            for _dir in (logs_dir, silo_dir, jenkins_node_dir):
                if not _tmpfile_upload(_dir, file):
                    return False
            return True

        if mimetypes.guess_type(file)[0] is None and mimetypes.guess_type(file)[1] is None:
            return _tmpfile_upload(s3_path, file, {"ContentType": "text/plain"})

        if mimetypes.guess_type(file)[0] is None:
            return _tmpfile_upload(
                s3_path, file, {"ContentType": "text/plain", "ContentEncoding": mimetypes.guess_type(file)[1]}
            )

        return _tmpfile_upload(s3_path, file)

    def _deploy_s3_upload(file):
        log.info("Attempting to upload file {}".format(file))
        return _s3_put_file(file)
