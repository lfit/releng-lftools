...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    def _s3_put_file(_dir, file, ExtraArgs=None):
        if ExtraArgs:
            try:
                s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(_dir, file), ExtraArgs)
                # print("s3.Bucket("+s3_bucket+").upload_file("+file+", {}{}.format("+_dir+", "+file+"), "+ExtraArgs+")")
                # s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(_dir, file), ExtraArgs={"ContentType": "text/plain", "ContentEncoding": mimetypes.guess_type(file)[1]})
                # print("s3.Bucket("+s3_bucket+").upload_file("+file+", {}{}.format("+_dir+", "+file+"), ExtraArgs={\"ContentType\": \"text/plain\", \"ContentEncoding\": "+mimetypes.guess_type(file)[1]+"})")
                # print("{}{} args= {}".format(_dir, file, _ExtraArgs))
            except ClientError as e:
                log.error(e)
                return False
            return True
            ...
            try:
                # print("{}{} has no args".format(_dir, file))
                s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(_dir, file))
            except ClientError as e:
                log.error(e)
                return False
    ...
    def _s3_file_type(file):
        if file == "_tmpfile":
            ...
            args = 'ExtraArgs={"ContentType": "text/plain"}'
            _s3_put_file(s3_path, file, args)
        else:
            args = 'ExtraArgs={"ContentType": "text/plain", "ContentEncoding": ' + mimetypes.guess_type(file)[1] + "}"
            _s3_put_file(s3_path, file, args)
