...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    def _s3_put_file(file):
        if file in "_tmpfile":
            try:
                s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(logs_dir, file))
            except ClientError as e:
                log.error(e)
                return False
            else:
                try:
                    s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(silo_dir, file))
                except ClientError as e:
                    log.error(e)
                    return False
                else:
                    try:
                        s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(jenkins_node_dir, file))
                    except ClientError as e:
                        log.error(e)
                        return False
            return True
