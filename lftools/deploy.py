...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
        ...
        if file in "_tmpfile":
            try:
                ...
                try:
                    ...
                    try:
                        s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(s3_bucket, file))
                    except ClientError as e:
                        log.error(e)
                        return False
            return True
    ...
    # Cleanup
    # s3.Object(s3_bucket, "{}{}".format(s3_bucket, "_tmpfile")).delete()
    s3.Object(s3_bucket, "{}{}".format(silo, "_tmpfile")).delete()
    s3.Object(s3_bucket, "{}{}".format(jenkins_node, "_tmpfile")).delete()
    os.chdir(previous_dir)
    shutil.rmtree(work_dir)
