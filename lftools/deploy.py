...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    # Create, upload & delete dummy files to create directory structure
    s3_silo = s3_path.split("/")[0] + "/"
    s3_jenkins_host = s3_silo + s3_path.split("/")[1] + "/"
    try:
        s3.Object(s3_bucket, "{}{}".format(s3_silo, "index.html")).load()
    except botocore.exceptions.ClientError as e:
        if e.response["Error"]["Code"] == "404":
            log.info("Creating dummy file")
            open("test.txt", "a").close()
            log.info("Uploading dummy file to {}".format(s3_silo))
            s3.Bucket(s3_bucket).upload_file("test.txt", "{}{}".format(s3_silo, "test.txt"))
    else:
        log.info("{}{}".format(s3_silo, "index.html") + " exists!")

    try:
        s3.Object(s3_bucket, "{}{}".format(s3_jenkins_host, "index.html")).load()
    except botocore.exceptions.ClientError as e:
        if e.response["Error"]["Code"] == "404":
            log.info("Uploading dummy file to {}".format(s3_jenkins_host))
            s3.Bucket(s3_bucket).upload_file("test.txt", "{}{}{}".format(s3_silo, s3_jenkins_host, "test.txt"))
    else:
        log.info("{}{}".format(s3_jenkins_host, "index.html") + " exists!")
