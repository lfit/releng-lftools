...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    log.info("Creating _tmpfile")
    io.open("_tmpfile", "a").close()
    ...
    def _s3_put_file(file):
        if mimetypes.guess_type(file)[0] is None and mimetypes.guess_type(file)[1] is None:
            ...
            try:
                ...
                return False
            return True
        elif file in "_tmpfile":
            log.info("Found _tmpfile")
            silo = s3_path.split("/")[0] + "/"
            jenkins_node = silo + s3_path.split("/")[1] + "/"
            try:
                log.info("Uploading _tmpfile to {}".format(silo))
                s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(silo, file))
            except ClientError as e:
                log.error(e)
                return False
            else:
                try:
                    log.info("Uploading _tmpfile to {}".format(jenkins_node))
                    s3.Bucket(s3_bucket).upload_file(file, "{}{}".format(jenkins_node, file))
                except ClientError as e:
                    log.error(e)
                    return False
            return True
    ...
    # Create & upload tmp files to create directory structure
    """
    log.info("Creating tmp file")
    tmp_file=open(".tmp_file", "a")
    log.info("Uploading file {} to {}".format(tmp_file.name, silo))
    try:
        s3.Bucket(s3_bucket).upload_file("{}".format(tmp_file), "{}{}".format(silo, tmp_file))
    except botocore.exceptions.ClientError as e:
        log.info(e)
    log.info("Uploading file {} to {}".format(tmp_file.name, jenkins_node))
    try:
        s3.Bucket(s3_bucket).upload_file("{}".format(tmp_file), "{}{}".format(jenkins_node, tmp_file))
    except botocore.exceptions.ClientError as e:
        log.info(e)

    # Delete dummy file
    s3.Object(s3_bucket, "{}{}".format(silo, tmp_file.name)).delete()
    s3.Object(s3_bucket, "{}{}".format(jenkins_node, tmp_file.name)).delete()
    """
    log.info("#######################################################")
