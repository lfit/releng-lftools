...
def deploy_s3(s3_bucket, s3_path, build_url, workspace, pattern=None):
    ...
    open("silo", "a").close()
    open("jenkins_host", "a").close()
    log.info("Uploading file {} to {}".format("silo", s3_silo))
    s3.Bucket(s3_bucket).upload_file("silo", "{}{}".format(s3_silo, "silo"))
    log.info("Uploading file {} to {}".format("jenkins_host", s3_jenkins_host))
    s3.Bucket(s3_bucket).upload_file("jenkins_host", "{}{}{}".format(s3_silo, s3_jenkins_host, "jenkins_host"))
