...
def helper_user_github(ctx, organization, user, team, delete, admin, single):
    ...
    try:
        ...
        if single:
            log.info("    - name: '{}'".format(user_object.name))
            log.info("      email: '{}'".format(user_object.email))
            log.info("      company: '{}'".format(user_object.company))
            log.info("      id: '{}'".format(user))
            log.info("      company: 'Unknown/Unknown'")
            sys.exit(0)
    except GithubException as ghe:
        log.error(ghe)
