...
def create_ldap_groups_ids(config_file, settings_file):
    ...
    if 'groups_base' not in config['ldap']:
        print('ERROR: LDAP project group base dn needs to be defined')
        error = True

    if 'users_base' not in config['ldap']:
        print('ERROR: LDAP users base dn needs to be defined')
        error = True

    if 'project_name' not in config['ldap']:
        print('ERROR: project name needs to be defined')
        error = True

    if 'master_project' not in config['ldap']:
        print('ERROR: master project name needs to be defined')
        error = True

    if 'members' not in config['ldap']:
    ...
    try:
        ...
        dn = "cn=" + str(config['ldap']['master_project']) + "-gerrit-" + \
        ...
        attrs['cn'] = str(config['ldap']['master_project']) + "-gerrit-" + \
                      str(config['ldap']['project_name']) + "-committers"
        member_list = []
