...
def audit(ctx, organization):
    ...
    print("Outside collaborators without 2fa:\n", orgName)
    try:
...
@click.argument('organization')
@click.pass_context
def list_groups(ctx, organization):
    """List teams and their users for an Org."""
    token = config.get_setting("github", "token")
    g = Github(token)
    orgName = organization
    try:
...
github_cli.add_command(list_groups)
