...
@click.pass_context
def create_webhooks(ctx, config, settings, repository):
...
@click.option('-r', '--repository', type=str, required=True,
              help="Name of the Github repository.")
@click.pass_context
def protect_branches(ctx, config, settings, repository):
    """Protect branches in a Github repository."""
    githubcmd.protect_branches(config, settings, repository)
