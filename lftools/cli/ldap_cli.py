...
@click.pass_context
def verifyinfofile(ctx, gerritclonebase, ldapgroup, repo):
    """
    Verify INFO.yaml against LDAP group
    """
    params = ['gerritclonebase', 'ldapgroup', 'repo']
    status = subprocess.call(['verifyinfofile', gerritclonebase, ldapgroup, repo])
    sys.exit(status)
...
ldap_cli.add_command(verifyinfofile)
