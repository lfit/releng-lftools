...
@click.option('--purpose', envvar='purpose', type=str, required=True,
              help='')
@click.option('--review', envvar='review', type=str, required=True,
              help='')
@click.pass_context
def autocorrectinfofile(ctx, gerritclonebase, ldapgroup, repo, purpose, review):
    """Verify INFO.yaml against LDAP group."""
    status = subprocess.call(['autocorrectinfofile', gerritclonebase, ldapgroup, repo, purpose, review])
    sys.exit(status)
