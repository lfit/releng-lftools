...
env = Environment(loader=PackageLoader("lftools.jenkins", "templates"), autoescape=select_autoescape(["html", "xml"]))
...
@click.command()
@click.option(
    "--projects", type=str, required=True, help="Yaml file containing the hierarchy of files and passwords to " "create"
)
@click.option(
    "--servers",
    "-s",
    ...
    multiple=True,
    required=False,
    help="ServerIDs to define in the settings files. Defaults to " "releases, staging, site, snapshots",
)
@click.pass_context
def create_settings(ctx, projects, servers):
    ...
    if not servers:
        servers = ["releases", "staging", "site", "snapshots"]

    template = env.get_template("create-settings-file.groovy")

    jenkins = ctx.obj["jenkins"]
    result = jenkins.server.run_script(template.render(projects=project_map, servers=servers))
    log.info(result)
