...
def checkmark(truthy):
    """Return a UTF-8 Checkmark or Cross depending on the truthiness of
    the argument"""
    if truthy:
        return u'\u2713'
    return u'\u2717'
...
def list_plugins(ctx):
    ...
    for key in plugins.keys():
        ...
        print("%s [%s] [%s] [%s] [%s]" % (plugin_name,
            plugin['version'], plugin['enabled'],
            plugin['active'], plugin['hasUpdate']))
