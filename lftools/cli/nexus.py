...
def docker_params(command):
    ...
    command = click.option(
        '-s', '--server', type=str,
        help='Nexus server URL. Can also be set as {} in the environment. This '
        + 'will override any URL set in settings.yaml.'.format(
...
def delete_images(ctx, settings, server, repo, pattern, yes):
    ...
    if yes or click.confirm("Would you like to delete all " + str(len(images))
                            + " images?"):
        nexuscmd.delete_images(settings, server, images, yes)
