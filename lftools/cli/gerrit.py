...
def checkproject(ctx, gerrit_url, gerrit_project):
    ...
    list = [ 'projects/', 'projects/{}'.format(gerrit_project) ]
    #check if gerrit exists, check if project exists
    for access_str in list:
        try:
            result = rest.get(access_str, headers=headers)
        except:
...
#this works, but I want to force push a .gitreview, which I'm not sure about via the api.
#maybe I just submit it via the api. that seems doable.
#https://stackoverflow.com/questions/50361450/how-to-create-change-using-gerrit-in-browser
#I would also like a way to enable github replication.
#but some file has to be submitted first.
#end goal here is to do this all from an info file..
