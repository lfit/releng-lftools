...
@click.pass_context
def addinfojob(ctx, gerrit_url, gerrit_project, jjbrepo):
    ...
    payload = json.dumps({
        ...
        "subject": 'Automation adds jjb INFO Job\n\nSigned-off-by: {}'.format(signed_off_by),
        "branch": 'master',
        })
    print(payload)
    access_str = 'changes/'
...
- project:
    ...
    access_str = 'changes/{0}/edit/jjb%2F{1}%2Finfo-{2}.yaml'.format(changeid, gerrit_project_encoded, gerrit_project_dashed)
