...
def archives(ctx, logs_url, logs_path, workspace, tmpdir, pattern):
    ...
    To use this command for S3, -tmpdir/--tmpdir must be specified.
    """
    if not pattern:
        pattern = None
...
def logs(ctx, logs_url, logs_path, build_url, tmpdir):
    """Deploy logs to a Nexus site repository or a S3 bucket.
    ...
    To use this for S3 log shipping, the S3 bucket must be exist and the -tmpdir/--tmpdir
    must be specified.
    """
    try:
        if 'nexus' in logs_url:
