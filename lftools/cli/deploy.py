...
@click.option('-s', '--settings-file', envvar='SETTINGS_FILE', default='',
              help='Settings file.')
@click.pass_context
def maven_file(ctx, nexus_url, repo_id, group_id, file_name, maven_bin, classifier, global_settings_file, settings_file):
    ...
    status = subprocess.call(['deploy', 'maven-file', nexus_url, repo_id, group_id, file_name,
                              maven_bin, classifier, global_settings_file, settings_file])
    sys.exit(status)
