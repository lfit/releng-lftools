...
def archives(ctx, logs_url, logs_path, workspace, pattern):
    """Archive files to a Nexus site repository or a S3 bucket.
    ...
    To use this command for Nexus, the Nexus server must have a site repository
    configured with the name "logs" as this is a hardcoded log path.
...
@click.pass_context
def file(ctx, nexus_url, nexus_repo_id, group_id, artifact_id, version,
    ...
    """
    try:
        deploy_sys.upload_maven_file_to_nexus(nexus_url, nexus_repo_id,
...
def logs(ctx, logs_url, logs_path, build_url):
    ...
    To use this script for Nexus logs, the Nexus server must have a site repository
    configured with the name "logs" as this is a hardcoded path..
...
@click.argument('s3_bucket', envvar='LOGS_URL')
@click.argument('s3_key', envvar='LOGS_PATH')
@click.argument('build-url', envvar='BUILD_URL')
@click.argument('workspace', envvar='WORKSPACE')
@click.option('-p', '--pattern', multiple=True)
@click.pass_context
def logs_s3(ctx, s3_bucket, s3_key, build_url, workspace, pattern):
    """Deploy logs and archives to a S3 bucket."""
    if not pattern:
        pattern = None
    try:
        deploy_sys.deploy_logs_s3(s3_bucket, s3_key, build_url, workspace,
                                  pattern)
    except HTTPError as e:
        log.error(str(e))
        sys.exit(1)
...
# Maven Config
@click.option(
    '-b', '--maven-bin', envvar='MAVEN_BIN', help='Path of maven binary.')
@click.option(
    '-gs',
    '--global-settings',
...
@click.option(
    '-s', '--settings', envvar='SETTINGS_FILE', help='Settings file.')
@click.option(
    '-p',
    '--maven-params',
...
# Maven Artifact GAV
@click.option(
    '-a', '--artifact-id', help='Maven Artifact ID.')
@click.option('-c', '--classifier', help='File classifier.')
@click.option(
    '-f', '--pom-file', help='Pom file to extract GAV information from.')
...
def nexus(ctx, nexus_repo_url, deploy_dir, snapshot):
    ...
    """
    log.debug("nexus_repo_url={}, deploy_dir={}, snapshot={}".format(
...
@click.pass_context
def nexus_stage_repo_close(ctx, nexus_url, staging_profile_id,
                           staging_repo_id):
    """Close a Nexus staging repo."""
    deploy_sys.nexus_stage_repo_close(nexus_url, staging_profile_id,
                                      staging_repo_id)
...
def nexus_zip(ctx, nexus_url, nexus_repo, nexus_path, deploy_zip):
    ...
    """
    try:
        deploy_sys.deploy_nexus_zip(nexus_url, nexus_repo, nexus_path,
                                    deploy_zip)
    except IOError as e:
        log.error(str(e))
        sys.exit(1)
