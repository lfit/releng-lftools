...
@click.option('-gs', '--global-settings-file', type=str, required=False, default=None)
@click.option('-s', '--settings-file', type=str, required=False, default=None)
@click.pass_context
def maven_file(ctx, nexus_url, repo_id, file_name, global_settings_file, settings_file):
    ...
    """
    status = subprocess.call(['deploy', 'maven-file', nexus_url, repo_id, file_name, global_settings_file, settings_file])
    sys.exit(status)
