...
def cleanup(os_cloud, days=0):
    ...
    def _remove_clusters_from_cloud(clusters, cloud):
        log.info("Removing {} clusters from {}.".format(len(clusters), cloud.cloud_config.name))
        for cluster in clusters:
            try:
                ...
                if str(e).startswith("Multiple matches found for"):
                    log.warning("{}. Skipping cluster...".format(str(e)))
                    continue
                else:
            ...
            if not result:
                ...
                log.info('Removed "{}" from {}.'.format(cluster.name, cloud.cloud_config.name))
...
def remove(os_cloud, cluster_name, minutes=0):
    ...
    if minutes > 0 and created_time >= datetime.utcnow() - timedelta(minutes=minutes):
        log.warning('Cluster "{}" is not older than {} minutes.'.format(cluster.name, minutes))
    else:
        cloud.delete_coe_cluster(cluster.uuid)
        log.info('Deleted cluster "{}".'.format(cluster.name))
