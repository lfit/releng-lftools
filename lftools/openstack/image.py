...
def _filter_images(images, days=0, hide_public=False, only_ci_managed=True):
    """Filter image data and return list.

    :arg bool hide_public: Whether or not to include public images.
    :arg bool only_ci_managed: Filters images with metadata set to `ci_managed=yes`.
    """
    filtered = []
    for image in images:
...
def list(os_cloud, days=0, hide_public=False, only_ci_managed=True):
    """List images found according to parameters."""
    cloud = shade.openstack_cloud(cloud=os_cloud)
    images = cloud.list_images()

    filtered_images = _filter_images(images, days, hide_public, only_ci_managed)
    for image in filtered_images:
        print(image.name)


def cleanup(os_cloud, days=0, hide_public=False, only_ci_managed=True, clouds=None):
