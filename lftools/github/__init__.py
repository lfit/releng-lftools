...
class GitHub:
    ...
    def get_rate_limit_status(self, username):
        ...
        url = ''.join([self.baseurl, '/user'])
        rate_limit = requests.get(url, auth=self.auth, headers=self.headers)
        return rate_limit
    ...
    def get_org_repos(self, org):
        ...
        org_repos = requests.get(url, auth=self.auth, headers=self.headers)
        json_org_repos = []
        json_org_repos.append(org_repos)
        print(org_repos.links)
        if 'next' in org_repos.links or 'last' in org_repos.links:
            org_repos = requests.get(org_repos.links["next"]["url"],
                                     auth=self.auth,
                                     headers=self.headers)
            json_org_repos.append(org_repos)
        return json_org_repos

    def get_org_teams(self, org):
        """Get all the repositories of a GitHub organization"""
        url = ''.join([self.baseurl, '/orgs/%s/teams' % org])
    ...
    def get_repo_collaborators(self, org, repo):
        ...
        url = ''.join([self.baseurl, '/repos/%s/%s/collaborators?per_page=100' % (org, repo)])
        collaborators = requests.get(url, auth=self.auth, headers=self.headers).json()
        return collaborators

    def get_repo_contributors(self, org, repo):
        """Get all the collaborators of an organizations given repository"""
        url = ''.join([self.baseurl, '/repos/%s/%s/stats/contributors' % (org, repo)])
        print(url)
        contributors = requests.get(url, auth=None, headers=self.headers)
        print(contributors.links)
        return contributors
