...
class GitHub:
    ...
    def get_org_repos(self, org):
        ...
        url = ''.join([self.baseurl, '/orgs/%s/repos?per_page=100' % org])
    ...
    def get_repo_contributors(self, org, repo):
        """Get all the collaborators of an organizations given repository"""
        url = ''.join([self.baseurl, '/repos/%s/%s/stats/contributors?per_page=100' % (org, repo)])
        print(url)
        contributors = requests.get(url, auth=None, headers=self.headers).json()
        return contributors
